...
class State:  # 修改状态变量的类，下面训练模型的学习引擎继承了它
    """
...
class LearningEngine(State):
    r"""
    Base class for learning engine    学习引擎的基类
    ...
    [REQUIRED ARGS]  必要的参数：要训练的网络，损失函数，训练集加载器
    ...
    [OPTIONAL ARGS] 可选参数
        ...
        学习引擎本身包含一个state。训练期间涉及的类属性存储为状态变量。默认情况下，有7个状态变量:net、optimizer、epoch、iteration、running_loss、t_data、t_iteration。
        依次，它们指的是网络(nn.Module)、优化器(nn.Module)、epoch数(int)、迭代数(int)和记录运行损失、数据加载和迭代时间。
        可以使用方法state_dict()、load_state_dict()、fetch_state_key(key)和update_state_key(**kwargs)访问和更新状态变量。有关详细信息，请参阅方法部分。
        ...
        包含的参数有以下：
            ...
            train_loader:训练集的数据加载器，批量输入格式为INPUT_1，…， input_n，标签)。每个元素应该采取以下形式之一:张量，List[张量]，Dict[张量]

            optim:要使用的优化器。在“SGD”和“Adam”之间选择

            optim_params:所选优化器的参数。默认设置为学习速率为1e-3，动量为0.9，权值衰减为5e-4

            optim_state_dict:要加载的优化器状态字典

            lr_scheduler:如果为True，使用MultiStepLR作为学习率调度器

            lr_sched_params:学习率调度器的参数。

            verbal:如果为True，则每固定间隔打印一次统计信息

            print_interval:打印统计信息的迭代次数

            cache_dir:保存checkpoint模型的的目录

        包含的方法有：
            __call__(n: int) ->None:运行学习引擎n个epoch

            save_checkpoint() ->None:保存一个checkpoint，其中包含模型状态、优化器状态、epoch和iteration numbers。checkpoint保存在指定的缓存目录下，文件名包括iteration numbers以及epoch

            state_dict()—> dict:返回状态字典

            load_state_dict(dict_in: dict) ->None:从外部dict加载状态

            fetch_state_key(key: str)—> Any:返回指定的键

            update_state_key(**kwargs) ->None:覆盖状态中的特定键

            要从基类继承，请根据具体情况重写以下方法：上面定义了这个类就是基类

            _on_start():在训练前执行。此方法用于初始化

            _on_start_epoch():在每个epoch之前执行。在方法开始的时候，在子类中调用super()._on_start_epoch()

            _on_start_iteration():在每次迭代之前执行。在方法开始的时候，在子类中调用super()._on_start_iteration()

            _on_each_iteration():执行向前、向后传递和参数更新等。除非必要，否则不应该重写此方法

            _on_end_iteration():在每次迭代之后执行。在方法结束的时候，在子类中调用super()._on_end_iteration()

            _on_end_epoch():在每个epoch之后执行。在方法结束的时候，在子类中调用super()._on_end_epoch()
    ...
    def __call__(self, n: int) -> None:  # 模型进行训练，指定epoch
        self.num_epochs = n
    ...
    def _on_start_epoch(self): # 在每个epoch之前执行，epoch加一。
        self._state.epoch += 1

    def _on_end_epoch(self):  # 在每个epoch之后执行，保存训练好的模型到指定目录。
        self.save_checkpoint()
        if self._state.lr_scheduler is not None:
            self._state.lr_scheduler.step()

    def _on_start_iteration(self):  # 开始迭代
    ...
    def _print_statistics(self):  # 打印信息
    ...
    def save_checkpoint(self) -> None:  # 保存模型
    ...
    def save_snapshot(self) -> None:  # 保存一个快照，下次可以继续执行训练
