...
def compute_spatial_encodings(
    ...
    Parameters:
    -----------
        boxes_1: List[Tensor]  两组检测框
            First set of bounding boxes (M, 4)
        boxes_1: List[Tensor]
            Second set of bounding boxes (M, 4)
        shapes: List[Tuple[int, int]]
            Image shapes, heights followed by widths  图像的形状，高度和宽度
        eps: float
    ...
    for b1, b2, shape in zip(boxes_1, boxes_2, shapes):
        ...
        f = torch.stack([
            # Relative position of box centre  检测框中心的位置
            c1_x / w, c1_y / h, c2_x / w, c2_y / h,
            # Relative box width and height    检测框的宽度和高度
            b1_w / w, b1_h / h, b2_w / w, b2_h / h,
            # Relative box area                检测框的大小
            b1_w * b1_h / (h * w), b2_w * b2_h / (h * w),
            b2_w * b2_h / (b1_w * b1_h + eps),
            # Box aspect ratio                 检测框的纵横比
...
# 计算焦点损失，focal loss主要是为了解决目标检测中正负样本比例严重失衡的问题。该损失函数降低了大量简单负样本在训练中所占的权重
